...
public class GameplayBlackGammon extends Gameplay{
    ...
    public GameplayBlackGammon() {
        ...
        while(!start){
            rollDice();
            System.out.println(Colors.RED + " has rolled: " + dice.get(0));
            System.out.println(Colors.BLUE + " has rolled: " + dice.get(1));

            if(dice.get(0) > dice.get(1)){
                System.out.println("RED has higher number so, it is red's turn");
                boardBlackGammon.setCurrentPlayer(Colors.RED);
                start= true;
            }
            else if(dice.get(0) < dice.get(1)){
                System.out.println("BLUE has higher number so, it is blue's turn");
                boardBlackGammon.setCurrentPlayer(Colors.BLUE);
                start= true;
            }
            else{
                System.out.println("Both have rolled an equal equal number. So, another dice Roll.");
            }
            rollDice();
        }
    ...
    public void run() {
        ...
        while (skip) {
            ...
            boardBlackGammon.setCurrentPlayer(currentPlayer);
            boardBlackGammon.draw();
            rollDice();
            getInputLogic();
        }
        ...
        while (!boardBlackGammon.isWin(Colors.RED) && !boardBlackGammon.isWin(Colors.BLUE)) {
        ...
        if (boardBlackGammon.isWin(Colors.RED)) {
            System.out.println("Congratulations!!! Red has Won");
        }
        if (boardBlackGammon.isWin(Colors.BLUE)) {
        ...
        public GetInput invoke(int choice) {
            ...
            while (!canMove) {
                if (currentPlayer.equals(Colors.BLUE)) {
                    for (int i = 0; i <= 27; i++) {
                        if (boardBlackGammon.canMovewithoutMessage(i, Math.abs(i + numD) % 28)) {
                            canMove = true;
                        }
                    }
                } else {
